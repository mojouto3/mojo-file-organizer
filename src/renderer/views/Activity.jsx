...
const TYPE_LABEL = { rules: 'Rules', watcher: 'Watcher', cleanup: 'Cleanup', 'smart-group': 'Smart group' };
const TYPE_COLOR = {
...
function SessionCard({ session: s, expanded, onToggle, onChanged }) {
  ...
  const handleExport = () => {
    const lines = [
      ...
      '',
      isCleanup ? 'Files cleaned:' : 'Files moved:',
      ...(isCleanup
  ...
  const handleUndoSession = async () => {
    const ok = await confirm(`Undo this session (${sessionFileCount(s)} items)?`, { confirmLabel: 'Undo' });
    if (!ok) return;
    ...
    if (result.restored?.length > 0) {
      ...
      showToast(`Restored ${result.restored.length} item(s)`);
      onChanged();
    } else {
      showToast('Nothing could be restored');
    }
  };

  const handleDeleteSession = async () => {
    const ok = await confirm('Delete this session record?', { confirmLabel: 'Delete' });
    if (!ok) return;
    await window.api.deleteSession(s.id);
    onChanged();
  };
  ...
  return (
    <Card className="overflow-hidden">
      <div onClick={onToggle} className="flex cursor-pointer items-center gap-2.5 px-3.5 py-2.5">
        ...
        <span className="w-16 shrink-0 text-right text-[11px] text-mfo-text-dim">{sessionFileCount(s)} items</span>
        <div className="flex shrink-0 items-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => setEditingNote((v) => !v)} className="rounded p-1 text-mfo-text-dim hover:bg-mfo-surface2 hover:text-mfo-text" title="Note"><Pencil size={13} /></button>
          <button
          ...
          <button onClick={handleOpenFolder} className="rounded p-1 text-mfo-text-dim hover:bg-mfo-surface2 hover:text-mfo-text" title="Open folder"><FolderOpen size={13} /></button>
          <button
          ...
          <button onClick={handleDeleteSession} className="rounded p-1 text-mfo-text-dim hover:bg-mfo-danger/10 hover:text-mfo-danger" title="Delete"><X size={13} /></button>
        </div>
      ...
      {editingNote && (
        <div className="flex items-center gap-2 border-t border-mfo-border px-3.5 py-2" onClick={(e) => e.stopPropagation()}>
          ...
          />
          <Button variant="outline" onClick={saveNote} className="px-2.5 py-1">Save</Button>
        </div>
      )}
      {!editingNote && s.note && (
              ...
              <div className="flex flex-wrap gap-1.5">
                {files.map((f) => (
                  <span key={f.to || f.name} className="flex items-center gap-1.5 rounded-md border border-mfo-border px-2 py-1 text-[11px] text-mfo-text">
                    {f.name}
                    {f.to && (
                      <button onClick={() => window.api.openFileLocation(f.to)} className="text-mfo-text-dim hover:text-mfo-text" title="Open location">
                        <ExternalLink size={11} />
                      </button>
                    )}
                    <button onClick={() => handleUndoFile(f)} className="text-mfo-text-dim hover:text-mfo-green" title="Undo this file">
                      <Undo2 size={11} />
                    </button>
                  </span>
                ))}
                  ...
                  {ruleGroups[action].map((r, i) => (
                    <span key={i} className="flex items-center gap-1.5 rounded-md border border-mfo-border px-2 py-1 text-[11px] text-mfo-text">
                      ...
                      {action !== 'delete' && r.from && r.to && (
                        <button onClick={() => handleUndoFile(r)} className="text-mfo-text-dim hover:text-mfo-green" title="Undo this file">
                          <Undo2 size={11} />
                        </button>
                      )}
                    </span>
...
function SessionsView() {
  ...
  const clearAll = async () => {
    const ok = await confirm('Clear all history?', { confirmLabel: 'Clear' });
    if (!ok) return;
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={clearFilters} className="text-[11px] text-mfo-text-dim hover:text-mfo-text">Clear filters</button>
      </div>
          ...
          {TYPE_FILTERS.map((t) => (
            <button
              ...
              className={`rounded-md border px-2.5 py-1 text-xs font-medium capitalize transition-colors ${
                typeFilter === t ? 'border-mfo-green bg-mfo-green/10 text-mfo-green' : 'border-mfo-border text-mfo-text-dim hover:text-mfo-text'
              }`}
            >
      ...
      {filtered.length === 0 ? (
        <Card className="flex flex-col items-center gap-2 px-6 py-14 text-center">
          ...
          <p className="text-sm font-medium text-mfo-text">No history yet.</p>
          <p className="text-xs text-mfo-text-dim">Organize, clean, or run rules to see activity here.</p>
        </Card>
      ) : (
...
const WEEK_LABELS = ['7 weeks ago', '6 weeks ago', '5 weeks ago', '4 weeks ago', '3 weeks ago', '2 weeks ago', 'Last week', 'This week'];
  ...
  if (!hasOrganize && !hasRules) {
    return (
      <Card className="flex flex-col items-center gap-2 px-6 py-14 text-center">
        ...
        <p className="text-sm font-medium text-mfo-text">No stats yet.</p>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {hasOrganize && <StatCard label="Files organized" value={stats.totalFiles} accent />}
        {hasOrganize && <StatCard label="Organize sessions" value={stats.totalSessions} />}
        {hasOrganize && <StatCard label="Categories used" value={categories.length} />}
        {hasRules && <StatCard label="Files via rules" value={stats.rulesStats.total} accent />}
        {hasRules && <StatCard label="Rule sessions" value={stats.rulesStats.sessions} />}
        {hasRules && <StatCard label="Move / delete / rename" value={`${stats.rulesStats.move} / ${stats.rulesStats.delete} / ${stats.rulesStats.rename}`} />}
      </div>

      {categories.length > 0 && (
        <Card className="p-3.5">
          <p className="mb-2.5 text-[12.5px] font-medium text-mfo-text">By category</p>
          <div className="flex flex-col gap-2">
      ...
      {hasTrend && (
        <Card className="p-3.5">
          <p className="mb-2.5 text-[12.5px] font-medium text-mfo-text">Weekly trend</p>
          <div className="flex flex-col gap-2">
            {stats.weeklyTrend.map((v, i) => (
              <ChartRow key={i} label={WEEK_LABELS[i]} value={v} max={maxWeekly} color="#3ddb3d" />
            ))}
          </div>
        </Card>
      )}
