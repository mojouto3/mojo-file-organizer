...
### Fixed
- Security: URL validation in open-release-page handler (only github.com URLs accepted)
- Security: Command injection protection in schtasks handlers via input sanitization
- Security: save-settings now validates and whitelists allowed keys
- Performance: hashFile now uses streaming (64KB chunks) instead of loading entire file into RAM
- Performance: ipcRenderer.on listeners now use removeAllListeners to prevent accumulation
- Performance: merged duplicate keydown event listeners into one
- UI: Organize tab now shows an interactive empty state with folder icon and shortcut hints
- UI: Toggle off state is now visually distinct from radio buttons
- UI: Stats bars now show per-category colors and percentages
- UI: Cleanup treemap blocks now have a dark gradient overlay
- UI: Double checkmark in "up to date" toast fixed
- Dependencies: updated undici to patch 2 high severity vulnerabilities

## [3.11.0] - 2026-06-25

### Added
- Windows Explorer context menu: right-click any folder to see "Organize with Mojo". Uses HKCU registry (no admin required), toggled from Settings > General, cleaned up on uninstall.
- Keyboard shortcuts: Ctrl+1-8 (tabs), Ctrl+O (organize), Ctrl+Z (undo), Ctrl+P (preview), Ctrl+F (focus search), Esc (close dialog), ? (show shortcuts help).
- Shortcuts help modal: press ? or click the ? button in the titlebar.
- History search: filter sessions in real time by folder name or filename.
- Export session: download any organize session as a .txt file.

### Fixed
- Cleanup treemap legend now shows correct translated labels.
- Organize preview count and label now appear on the same line.
- XSS prevention: all filenames sanitized before inserting into innerHTML.

### Changed
- Version badge clickable: shows toast with update check result.
- Tab switch has a subtle fade animation.
- Drag and drop shows a visual overlay.
- Recycle Bin shows item count alongside size.
- Reset button in Categories uses subtle neutral styling.
